// ========== 文件加密工具 (AES-256-GCM + 结构化文件头 + PBKDF2) ==========
...
const HDR_MIN_SIZE = 40; // 12 固定 + 12 IV + 16 Tag

/**
 * 文件头结构: 0-7 MAGIC | 8-9 version(BE) | 10 ivLen | 11 tagLen | 12-23 IV | 24-39 Tag | 40+ 密文
 */
function parseEncFileHeader(filePath) {
  ...
  if (ivLen !== 12 || tagLen !== 16) return { valid: false, reason: 'IV/Tag 长度异常' };
  return {
    valid: true, version,
    iv: hdr.slice(12, 24),
    tag: hdr.slice(24, 40),
...
function encryptFileStream(inputPath, outputPath, aesKey) {
  return new Promise((resolve, reject) => {
    ...
    output.on('error', err => { if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath); reject(err); });
    input.on('error', err => { if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath); reject(err); });
    cipher.on('error', err => { if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath); reject(err); });
  });
}
