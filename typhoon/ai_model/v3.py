...
# -*- coding: utf-8 -*-
"""
方案二+三增强版 V3：特征加权训练 + 强化物理约束损失函数
============================================
优化升级点对比V2：
1. 大幅提升引导气流辅助损失权重，强制贴合副高东风环流，抑制东向漂移
2. 加强副高场景违规样本惩罚力度，限制不合理东移、过度北跳
3. 提高L2正则与Dropout比例，减轻过拟合带来的多步滚动发散
4. 降低初始学习率，训练收敛更平缓，长时序推演稳定性提升
5. 延长早停等待轮数，充分学习环境环流特征
基础功能不变：
1. 输入9维特征 [lon,lat,wind,pressure,hgt500,u500,v500,ridge_lat,west_extent]
2. 副高特征时序注意力自适应加权
3. 物理引导气流方向惩罚（样本权重实现）
4. 双输出：台风位移增量 + 500hPa引导气流辅助预测
...
MODEL_PATH_V3 = os.path.join(MODEL_DIR, 'july_track_model_v3.h5')
SCALER_PATH_V3 = os.path.join(MODEL_DIR, 'track_scaler_v3.pkl')
...
# 物理约束权重（V3强化）
LAMBDA_PHYSICS = 0.6    # V2原0.3，翻倍增强物理惩罚
LAMBDA_STEER = 0.4      # V2原0.1，大幅提升引导气流监督力度
...
def build_physics_penalty_mask(df):
    ...
    for i, row in df.iterrows():
        ...
        if east_ridge and easterly and west_extend:
            mask[i] = 1  # 标记为副高强场景
            # V3升级：提高违规惩罚倍数，强化西移约束
            target_dlon = row.get('dlon', 0)
            if target_dlon > 0:
                penalty[i] = 4.0  # V2原3.0 → 4倍惩罚，严厉抑制东行
            target_dlat = row.get('dlat', 0)
            if target_dlat > 0.5:
                penalty[i] = max(penalty[i], 2.5)  # V2原2.0提升至2.5
...
def build_model_v3():
    """
    构建增强版GRU模型 V3（带特征加权+强化物理约束）。
    
    架构设计：
    1. 输入层: 9维特征 [lon,lat,wind,pressure, hgt500,u500,v500,ridge_lat,west_extent]
    2. 特征分离: 将副高特征从输入中分离，通过注意力层加权
    3. GRU编码: 3层GRU (128→64→32)，V3提升L2正则与Dropout抑制过拟合
    4. 辅助引导气流头: 从GRU隐含状态预测引导气流方向，V3加大损失权重
    5. 输出层: [dlon, dlat, dwind]
    6. 物理约束损失: 在训练时对违反引导气流的方向施加更强惩罚
    """
    import keras
    ...
    # GRU编码 V3：加大L2正则、提升Dropout
    gru1 = GRU(128, return_sequences=True, kernel_regularizer=l2(2e-5),
               name='gru_1')(combined)
    bn1 = BatchNormalization(name='bn_1')(gru1)
    dp1 = Dropout(0.3, name='drop_1')(bn1)

    gru2 = GRU(64, return_sequences=True, kernel_regularizer=l2(2e-5),
               name='gru_2')(dp1)
    bn2 = BatchNormalization(name='bn_2')(gru2)
    dp2 = Dropout(0.3, name='drop_2')(bn2)

    gru3 = GRU(32, return_sequences=False, kernel_regularizer=l2(2e-5),
               name='gru_3')(dp2)
    dp3 = Dropout(0.25, name='drop_3')(gru3)
...
def train_model_v3():
    """V3主训练流程"""
    log("=" * 60)
    log("台风路径AI预测模型 v3 - 强化副高物理约束+环流辅助损失")
    log("=" * 60)
    ...
    # 准备辅助引导气流标签
    steer_train = X_train[:, -1, BASE_FEATURES:BASE_FEATURES + 2]
    ...
    callbacks = [
        keras.callbacks.EarlyStopping(
            monitor='val_main_output_loss', patience=20,
    ...
    log(f"V3测试集总损失: {test_metrics[0]:.6f}")

    # 保存V3独立模型，不覆盖V2旧权重
    model.save(MODEL_PATH_V3)
    log(f"V3模型保存: {MODEL_PATH_V3}")
    log(f"V3标准化器保存: {SCALER_PATH_V3}")
