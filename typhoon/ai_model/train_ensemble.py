...
架构：GRU(384→192→96) + MultiHeadAttn, 6时步
"""
...
MODEL_DIR = r"D:/AI_Model/ensemble"
SCALER_SAVE = r"D:/AI_Model/track_scaler_ensemble.pkl"
TIMESTEPS = 6
FEAT_DIM = 9
BATCH_SIZE = 256
EPOCH_MAX = 300
...
# ===================== 数据增强 =====================
def augment_storm(seq, targets):
    """数据增强：随机旋转+噪声"""
    angle = np.random.uniform(-np.pi/6, np.pi/6)  # ±30度
    cos_a, sin_a = np.cos(angle), np.sin(angle)
    # 旋转经纬度
    seq_aug = seq.copy()
    lons = seq_aug[:, 0] * cos_a - seq_aug[:, 1] * sin_a
    lats = seq_aug[:, 0] * sin_a + seq_aug[:, 1] * cos_a
    seq_aug[:, 0] = lons
    seq_aug[:, 1] = lats
    # 目标也旋转
    targets_aug = targets.copy()
    for i in range(0, len(targets), 4):
        dl = targets[i]; db = targets[i+1]
        new_dl = dl * cos_a - db * sin_a
        new_db = dl * sin_a + db * cos_a
        targets_aug[i] = new_dl
        targets_aug[i+1] = new_db
    return seq_aug, targets_aug
...
class MultiHeadLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        l24=tf.reduce_mean(tf.square(pr24-gt24)); l36=tf.reduce_mean(tf.square(pr36-gt36))
...
def build_gru_model(seed=42):
    ...
    g1 = GRU(384, return_sequences=True, kernel_regularizer=tf.keras.regularizers.L2(L2_COEFF))(inp)
    b1 = BatchNormalization()(g1); d1 = Dropout(DROPOUT_RATE)(b1)
    attn = MultiHeadAttn(head=4, units=72)(d1)
    g2 = GRU(192, return_sequences=True, kernel_regularizer=tf.keras.regularizers.L2(L2_COEFF))(attn)
    b2 = BatchNormalization()(g2); d2 = Dropout(DROPOUT_RATE)(b2)
    g3 = GRU(96, return_sequences=False, kernel_regularizer=tf.keras.regularizers.L2(L2_COEFF))(d2)
    b3 = BatchNormalization()(g3); d3 = Dropout(DROPOUT_RATE)(b3)
    shared = Dense(256, activation="relu")(d3); shared = Dropout(0.15)(shared)
    h6=Dense(96,activation="relu")(shared); h12=Dense(96,activation="relu")(shared)
    h24=Dense(96,activation="relu")(shared); h36=Dense(96,activation="relu")(shared)
    h72=Dense(128,activation="relu")(shared)
...
if __name__ == "__main__":
    print("=" * 60)
    print("[V5 Ensemble] 集成学习 - 3个GRU模型取平均")
    print("=" * 60)
    ...
    for i, m in enumerate(models):
        print(f"\n模型 {i+1}:")
        errs = evaluate_ensemble([m], sx, sy, df_test, n=300)
        all_individual_errs.append(errs)

    print("\n" + "="*60)
    print("集成模型 (3模型平均):")
    print("="*60)
    ensemble_errs = evaluate_ensemble(models, sx, sy, df_test, n=300)
